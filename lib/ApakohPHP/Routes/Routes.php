...
class Routes extends ModuleBase {
  ...
  public function getLink($route = null) {
    if (!isset($route)) {
      ...
      if (isset($route['path'])) {
        return $this->m
          ->Http
          ->getLink($route['path'], $route['query'], $route['fragment']);
      }
      ...
      return $this->m
        ->Http
        ->getLink(
          $this->getPath($route['controller'], $route['action'],
              $route['parameters']), $route['query'], $route['fragment']);
    }
    else {
      return $route;
    }
  ...
  public function redirect($route = null) {
    $this->m
      ->Http
      ->redirect(303, $this->getLink($route));
  }

  public function moved($route = null) {
    $this->m
      ->Http
      ->redirect(301, $this->getLink($route));
  }

  public function refresh($query = null, $fragment = null) {
    $this->m
      ->Http
      ->refreshPath($query, $fragment);
  }
  ...
  public function callController($sender, $eventArgs) {
    ...
    if (isset($this->selectedRoute)
      ...
      $this->m
        ->Errors
        ->fatal('Invalid controller',
          tr('%1::%2 is not valid', get_class($this->selectedController[0]),
