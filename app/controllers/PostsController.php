...
class PostsController extends AppController {
  ...
  public function preRender() {
    ...
    $this->Filtering
      ->addFilterColumn('status');
    $this->Filtering
      ->addFilterColumn('date');

    $this->Filtering
      ->addPredefined(tr('Published'), 'status:published');
    $this->Filtering
      ->addPredefined(tr('Draft'), 'status:draft');
    ...
    $this->Bulk
      ->addDeleteAction('delete', tr('Delete'));
  }
    ...
    if ($this->auth->hasPermission('frontend.posts.comments.add')) {
      if ($this->request->isPost() AND $this->request->checkToken()) {
        ...
        if (isset($this->request
          ->cookies['comment_author'])) {
          $this->newComment
            ->author = $this->request
            ->cookies['comment_author'];
        }
        if (isset($this->request
          ->cookies['comment_email'])) {
          $this->newComment
            ->email = $this->request
            ->cookies['comment_email'];
        }
  ...
  public function manage() {
    $this->Backend
      ->requireAuth('backend.posts.manage');
  ...
  public function add() {
    ...
    Post::setFieldEditor('content',
      $this->m
        ->Editors
        ->getEditor($this->config['editor']));

    if ($this->request
      ->isPost() AND $this->request
          ->checkToken('post')) {
      $this->post = Post::create($this->request
        ->data['post']);
      if (isset($this->request
        ->data['publish'])) {
        $this->post
  ...
  public function edit($post = null) {
    $this->Backend
      ->requireAuth('backend.posts.edit');
    ...
    if (!$this->request
      ...
      $this->Json
        ->respond(
          array('html' => $this->render('posts/post.html', true)));
    }
  }

  public function delete($post) {
    $this->Backend
      ->requireAuth('backend.posts.delete');
  ...
  public function viewTag($tag) {
    $this->tag = Tag::first(
      SelectQuery::create()->where('name = ?', $tag));
  ...
  public function manageTags() {
    $this->Backend
      ->requireAuth('backend.tags.manage');
    $this->title = tr('Tags');
    $this->tags = Tag::all();
    $this->render();
  }
