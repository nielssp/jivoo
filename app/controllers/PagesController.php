...
class PagesController extends AppController {
  ...
  public function view($page) {
    ...
    if (!$this->page
        OR ($this->page
          ->state != 'published'
            AND !$this->auth
              ->hasPermission('backend.pages.viewDraft'))) {
  ...
  public function add() {
    ...
    if ($this->request
      ->isPost()) {
      $this->page = Page::create($this->request
        ->data['page']);
  ...
  public function manage() {
    $this->Backend
      ->requireAuth('backend.pages.manage');
    ...
    if (isset($this->request
      ...
      $select->where('content LIKE ? OR title LIKE ?')
        ...
        ->addVar('%' . $this->filter . '%');
      $this->Pagination
        ->setCount(Page::count(clone $select));
    }
    else {
      $this->Pagination
        ->setCount(Page::count());
  ...
  public function delete($page) {
    $this->Backend
      ->requireAuth('backend.pages.delete');
