...
$(function() {
  ...
  $("#backend-toolbar a, #backend-page button, a.backend-button").each(function(index, Element) {
  ...
  $("#backend-toolbar span.menu").buttonset().children().next().button("option", {text : false});
  ...
  $("#backend-page-close").button({
    ...
    text: false
  }).removeClass('ui-button').removeClass('ui-state-default').addClass('ui-dialog-titlebar-close');
  ...
  $(".backend-permalink").each(function(index, Element) {
    var input = $(this);
    ...
    var icon;
    if (!titleId || (title.val() != '' && input.val() != '' && input.val() != title.val())) {
  ...
  $('textarea.backend-wysiwyg').tinymce({
